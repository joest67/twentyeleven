...
/**
 * Adds Travel_Widget widget.
 */
class Travel_Widget extends WP_Widget {
	...
	function __construct() {
		parent::__construct(
			'travel_widget', // Base ID
			'Travel_Widget', // Name
			array( 'description' => __( 'Posts about Travel  Widget', 'text_domain' ), ) // Args
		);
	}
	...
	public function widget( $args, $instance ) {
		...
		$travel_posts = new WP_Query( array( 'category_name' => 'travel', 'orderby' => 'title', 'order' => 'DESC' ) ); 
		echo $args['before_widget'];
		if ( ! empty( $title ) )
			echo $args['before_title'] . $title . $args['after_title'];
		while ($travel_posts->have_posts()) : $travel_posts->the_post();
		echo '<a href="';
                echo the_permalink();
		echo '">';
                echo the_title();
